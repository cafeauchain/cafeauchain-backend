module ImportServices
  class ImportLots
    ...
    def self.import(roaster_profile_id, json_object)
      ...
      LedgerServices::AssetTransferTransaction.new((json_object["key_pounds_on_hand"].to_f + json_object["key_pounds_roasted_to_date"].to_f), @lot.id, @roaster.id).call
      LedgerServices::RoastTransaction.new(json_object["key_pounds_roasted_to_date"].to_f, @batch.id, @roaster.id).call
      if @lot.save
